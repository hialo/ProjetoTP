...
import Persistencia.RedeDST;
import Servico.Parametros;
import Servico.UtilArquivos;
import java.io.File;
...
public class Treinamento extends javax.swing.JFrame {
    ...
    private void botao_treinarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botao_treinarActionPerformed
        new RedeDST(combo_funcao.getSelectedIndex());
        ...
        if (text_momento.getText().isEmpty() || text_taxa_aprendizado.getText().isEmpty() || text_numero_epocas.getText().isEmpty()) {
            ...
            RedeDST.dst.setEta(Double.parseDouble(text_taxa_aprendizado.getText()));
            RedeDST.dst.setAlpha(Double.parseDouble(text_momento.getText()));
            ...
            while (aux > Double.parseDouble((text_numero_epocas.getText()))) {
                ...
                for (int j = 0; j < Parametros.entradas_teste.length; j++) {
                    erro += RedeDST.dst.treinar(Parametros.entradas_teste[j], Parametros.resultados_esperados[j]);
                }
                erro = erro / Parametros.entradas_teste.length;
                //escreve no arquivo
                if (epocas == 0) {
                    try {
                        UtilArquivos.escrever(dir, arq, epocas + 1 + " " + erro, false);
                    } catch (IOException e) {
                        JOptionPane.showMessageDialog(this, e.toString());
                    }
                } else {
                    try {
                        UtilArquivos.escrever(dir, arq, epocas + 1 + " " + erro, true);
                    } catch (IOException e) {
                        JOptionPane.showMessageDialog(this, e.toString());
                    }
                }
